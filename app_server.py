...
async def notify_admin_paid(provider: str, *, payment_id: str, user_id: int, amount_cents: int, currency: str, external_id: str | None = None):
    if not cfg.admin_chat_id:
        return
    ext = f"\nexternal: <code>{external_id}</code>" if external_id else ""
    text = (
    ...
    try:
        await tg_app.bot.send_message(chat_id=cfg.admin_chat_id, text=text, parse_mode="HTML")
    except Exception:
        return
...
async def mp_webhook(request: Request):
    ...
    if status == "paid":
        ...
        if user_id:
            ...
            try:
                amt, cur = db.get_payment_amount(internal_id)
            except Exception:
                amt, cur = (0, "")
            await notify_admin_paid("mercadopago_pix", payment_id=internal_id, user_id=int(user_id), amount_cents=amt, currency=(cur or "BRL"), external_id=external_id)
    ...
    if status == "paid":
        ...
        if user_id:
            ...
            try:
                ...
                amt, cur = (0, "")
            await notify_admin_paid("yookassa", payment_id=internal_id, user_id=int(user_id), amount_cents=amt, currency=(cur or "RUB"), external_id=external_id)
        return {"ok": True, "paid": True}
